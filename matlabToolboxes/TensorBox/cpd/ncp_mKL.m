function [P,output] = cp_mKL(X,R,opts)
% Fast Multiplicative Algorithm based on Kullback-Leiber divergence for
% nonnegative CP factorizes the N-way nonnegative tensor X  into
% nonnegative factors of R components. 
%
% INPUT:
...
for iter = 1:param.maxiters
    ...
    for n = updateorder(1:end)
        ...
        U{n} = U{n} .* G;
        if n~= N
            %bsxfun(@rdivide,U{n},sum(U{N}) +eps);
            U{n} = max(eps,U{n});
    ...
    function [G,Pmat] = cp_gradient(A,n,Pmat)
        ...
        if n <= ns
            if n == ns
                ...
                Pmat = reshape(Xt.data,[],prod(I(right))); % Right-side projection
                Pmat = Pmat * KRP_right ;
            else
