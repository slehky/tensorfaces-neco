function [P,out] = ncp_r1_cg(Y,opts)
...
for kiter = 1:param.maxiters
    ...
    %    = |Y|_F^2 + prod_n  ([u -g]'*[u -g]) x [1;eta_n]^2 - 
    %           - 2 * W x ([1;eta_n]^(2)
    % 
    
    W = ttm(Y,cellfun(@(x,y) [x.^2 -x.*y -x.*y y.^2],up,g,'uni',0),'t');% 4 x 4 x 4
    [eta,fcost_eta] = stepsize_N(W,up,g);
       
    unew = up;
    for n = 1:N
        unew{n} = up{n} - g{n}*eta(n);
    end
    up = unew;
...
function [etaN,fcost_eta,Fcost_eta] = stepsize_N(W,up,g)
N = ndims(W);
% f2(eta1,eta2,eta3) =  W x_n [1 eta_n]...
% [eta,fcost_eta] = stepsize_1(W,up,g);
eta = 0;
etaN = eta*ones(1,N);
etaNx = arrayfun(@(x) [1;x],etaN,'uni',0);
etaNx = etaNx([1;1],:);
...
for n = 1:N 
    ...
    Wn = Wn'*Wn;
    f1(n,:) = gen_poly_kron_1x(Wn(:),4); % f1_n(eta_n) =
    f1v(n) = polyval(f1(n,:),etaN(n));
end


W2 = tensor(reshape(W,2*ones(1,2*N)));
maxiters = 5000;
Fcost_eta = [];
for kieta = 1:maxiters
    for n = 1:N
        Wn2 = ttv(W2,etaNx(:),-[2*n-1 2*n]);

        f2 = gen_poly_kron_1x(Wn2(:),2);
        fcost = f1(n,:)*prod(f1v([1:n-1 n+1:end]));

        fcost(3:5) = fcost(3:5) - 2*f2;
        ...
        Fcost_eta = [Fcost_eta fcost_eta];
    end
    if abs(Fcost_eta(end)-Fcost_eta(end-1))< 1e-7
        break
    end
end
...
%%
function [eta,fcost_eta] = stepsize_1(W,up,g)
N = ndims(W);
