function phi_ge = phi_right(U,X_mps,n,dir,phi_ge,tmode)
...
N = numel(U);
if nargin<6
    tmode = 'n';  % normal X_mps x U otherwise X_mps x U^T
end
if nargin<5  || isempty(phi_ge) 
    phi_ge = cell(N,1);
end
...
switch dir
    case 'R2L'
        ...
        if n<=N
            if n == N
                phi_ge{n} = squeeze(X_mps{n})*U{n};
            elseif isempty(phi_ge{n+1})
                for k = N:-1:n
                    if k == N
                        phi_ge{k} = squeeze(X_mps{k})*U{k};
                    else
                        phi_ge{k} = reshape(X_mps{k},size(X_mps{k},1),[])*khatrirao(phi_ge{k+1},U{k});
                    end
                end
            else
                phi_ge{n} = reshape(X_mps{n},size(X_mps{n},1),[])*khatrirao(phi_ge{n+1},U{n});
            end
        end
