function [T,Uinit,errar] = tucker_als(X,R,varargin)
...
for iter = 1:maxiters
    ...
    for n = dimorder(1:end)
        ...
        Utilde2 = permute(Utilde,[nm nmc]);
        Utilde2 = reshape(double(Utilde2),[In(nm(1))*In(nm(2)) prod(R(nmc))]);
        Utilde2 = Utilde2*Utilde2';
        [Z,s] = eig(Utilde2);
        Z = Z*diag(sqrt(diag(s)));
        Z = reshape(Z,In(nm(1)),In(nm(2)));
        [u,s,v] = svds(Z,R);
        U{nm(1)} = u;
        U{nm(2)} = v;
