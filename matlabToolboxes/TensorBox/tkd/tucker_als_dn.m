...
params.addParamValue('noiselevel',0,@isscalar); % standard deviation of noise (assume Gaussian noise)
params.addParamValue('exacterrorbound',false,@islogical); % standard deviation of noise (assume Gaussian noise)
...
noiselevel = params.Results.noiselevel;
exacterrorbound = params.Results.exacterrorbound;
...
if noiselevel>0
    accuracy = norm(X)^2 - noiselevel^2 * prod(size(X));
end
