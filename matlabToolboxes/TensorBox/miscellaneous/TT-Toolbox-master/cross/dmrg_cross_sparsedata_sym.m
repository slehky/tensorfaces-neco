...
% cores. Practical results show that this implementation yields lower
% approximation error than one with Rmat
%
...
sz=n;
if (isempty(y) )
    y=tt_rand(sz,d,no_eigs);
    %y=tt_rand(sz,d,[1 2*ones(1,d/2-1) no_eigs 2*ones(1,d/2-1) 1]);
end
if ( ~vectorized )
...
for i=d:-1:2
    ...
    ind_new=zeros(d-i+1,rnew); % (d-i+1) indices x r_i
    for s=1:rnew
        f_in=ind(s);
...
% for i = d:-1:d/2+1
%     index_array{i} = index_array{d-i+1};
% end
...
% index_array_rev{1} = zeros(0,ry(1));
% index_array_rev{d+1} = zeros(ry(d+1),0);
% for i = d-1:-1:2
%     ind_old=index_array_rev{i+2};
...
%         ind_new(:,s)=[js,ind_old(:,rs)'];
%     end
%     index_array_rev{i+1}=ind_new;
%     
% end

%%
% index_array(1:d/2) = index_array(d:-1:d/2+1);
% ry(1:d/2) = ry(d:-1:d/2+1);
% for k = 1:d/2
%     y{k} = permute(y{d-k+1},[3 2 1]);
% end
%%
...
%     rm1=rmat{i}; rm2=rmat{i+2};
    ...
    if vectorized
        ...
        big_index = big_index(:,end:-1:1);
        ix_ = big_index(:,1) + (big_index(:,2:end)-1)*cumprod(n(end:-1:2)');
        score_2 =elem(ix_(:));
        ...
        score_2 =elem(big_index(:,end:-1:1));
    end
    ...
    score_2=reshape(score_2,[ry(i+2)*n(i+1),n(i)*ry(i)]);
...
%     v=v/rmat{i+2}; v=reshape(v,[r,n(i+1)*ry(i+2)]);
    ...
    if ( dir == 1 )
        ...
        y{d-i+1} = permute(y{i},[3 2 1]);
        y{d-i} =  permute(y{i+1},[3 2 1]);
