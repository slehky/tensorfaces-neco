...
%  A is a Hermitian matrix (sparse)
% and x is a TT-block format
%
%
...
% end;
szA = size(A); % (n1 x n2 x ...x nd) x (m1 x m2 x ...x md)
d = numel(szA)/2;
n = szA(1:d);
...
% Fixed by Phan Anh Huy, Feb 4.2016.
% error at this step, because the last core may not be updated
% when the algorithm gets out of the loop.
% Shift Rank to the last core

% crx2 = crx;
% 
% %%
% crx = crx2; rx = [1 ;cellfun(@(x) size(x,1), crx2(2:end)) ;1]'
for k = i:d-1
    core_k1 = reshape(crx{k},rx(k)*n(k), rx(k+1), b);
    core_k1 = permute(core_k1,[1 3 2]);
    core_k1 = reshape(core_k1,[],rx(k+1));
    core_k2 = reshape(crx{k+1},rx(k+1), n(k+1)*rx(k+2));            
    
    xcore = reshape(core_k1*core_k2,rx(k)*n(k),[]);
    
    [core_k1,s,core_k2]=svd(xcore,'econ');
    
    s=diag(s); 
    rk=my_chop2(s,norm(s)*eps/sqrt(d-1)); % Truncation
    core_k1=core_k1(:,1:rk); core_k2=bsxfun(@times,core_k2(:,1:rk),s(1:rk)');
    
    rx(k+1) = rk;
    crx{k} = reshape(core_k1,rx(k),n(k), rx(k+1));
    crx{k+1} = reshape(core_k2',rx(k+1),b,n(k+1),rx(k+2));
    crx{k+1} = permute(crx{k+1},[1 3 4 2]);
end
...
% phixax(i) = rightreduce_matrix(phixax(i+1), core_x{i}, core_A(i), core_x{i}, ...
%               rank_x(i), n(i),rank_x(i+1), 1,rank_A(i),rankA(i+1), rank_x(i),n(i),rank_x(i+1));
%
%                                J
%                                |    
%      (U 1)-- r2--(U 2)-- r3--(U 3)-- r4--(U 4)--r5--(U 4)
%        |           |           |           |          |    
%       n1           n2          n3          n4         n5 
%        |           |           |           |          |      
%        +-----------+-----------A-----------+----------+
...
%       m1           m2          m3          m4         m5
%        |           |           |           |          |    
%      (V 1)-- s2--(V 2)-- s3--(V 3)-- s4--(V 4)--s5--(V 5)
%                                |
%                                K
%  A: tensor n1 x n2 x ... x nd x m1 x m2 x ... x md
%  X: tensor n1 x n2 x ... x ni x J x ni+1 
%
%  Perform rightreduce product W^T * A * X from the last core to core k
%  that is 
%        A 
%
