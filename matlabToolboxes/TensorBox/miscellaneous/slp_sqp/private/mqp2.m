...
if isempty(H)
   [s,f,exitflag,output,LAMBDA]=linprog(fp,A(p2,:),b(p2),A(p1,:),b(p1),vlb,vub,x0,options);
    ...
    if (numel(p1) == 1) && (p1 == 1) && isempty(p2)
        % only one equality constraint
        % %    min  1/2 * s'*H*s + fp'*s   subject to   A*s + b = 0
        % SOlve the linear system in the Newton method
        % [H   A]   [s]    [-fb]
        % [A'  0] * [u] =  [  b]
        % 
        vv = H\[fp A(1,:)'];
        ...
        LAMBDA.eqlin = lda;
        LAMBDA.ineqlin =[];
        LAMBDA.lower = zeros(numel(vlb),1);
        LAMBDA.upper = zeros(numel(vlb),1);
        %u = [lda;zeros(numel(s)*2,1)];
        exitflag = 1;
        
    elseif (numel(p2) == 1) && (p2 == 1) && isempty(p1)
        % only one inequality constraint
        % %    min  1/2 * s'*H*s + fp'*s   subject to   A*s + b <= 0
        % Check if -H\fb is a feasible point 
        s = -H\fp;
        if A(1,:)*s  - b(1) <=0 
            lda = 0;
        else
