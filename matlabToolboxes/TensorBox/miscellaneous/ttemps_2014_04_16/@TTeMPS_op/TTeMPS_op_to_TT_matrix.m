function [TT] = TTeMPS_op_to_TT_matrix( A )
    %TTeMPS_to_TT Convert to TT Toolbox matrix format.
    %   TT = TT_to_TTeMPS( A ) takes the TTeMPS operator A and converts it into
    ...
    % Bugs in header of routines % Phan Anh Huy, Sep 2014 
    % TTeMPS_op_to_TT_matrix(x) - > TTeMPS_op_to_TT_matrix( A )
    ...
    TT = tt_matrix( TT, A.size_col', A.size_row' );
