function x = orth_at( x, pos, dir )
    ...
    if strcmpi(dir, 'left')
        ...
        x.U{pos} = reshape( Q, [sz(1:3) size(Q,2)] );
    ...
    elseif strcmpi(dir, 'right') 
        ...
        x.U{pos} = reshape( Q',[size(Q,2) sz(2:end)]); % Fix error occuring when U{pos} is a fat matrix. The rank needs to be corrected. Phan Anh Huy, Sept.  2014
        x.U{pos-1} = tensorprod( x.U{pos-1}, R, 4); 
