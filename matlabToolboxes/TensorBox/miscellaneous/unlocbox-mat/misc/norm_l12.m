function n12 = norm_l12(x, g_d,g_t, weights2,weights1)
%NORM_L12 Norm L12
%
%   norm_l12(x, g_d,g_t, weights2,weights1) returns the norm L12 of x
%   Works also for overlapping group.
%
%
...
%       (default = 1)weights1
%
%   Url: http://unlocbox.sourceforge.net/doc//misc/norm_l12.php
...
if nargin<3, g_t=ones(size(g_d,1),length(x)); end
if nargin<5, weights1=ones(size(g_t,2),size(g_t,1)); end
if nargin<4, weights2=ones(length(x),size(g_t,1)); end

if size(g_d,1)>1
    n12=0;
    for ii=1:size(g_d,1);
        n12=n12+norm_l12(x,g_d(ii,:),g_t(ii,:),weights2(:,ii),weights1(:,ii));
    end
else
    ...
    if max(g_t)==min(g_t),
        ...
        W2=reshape(weights2(g_d),l,length(x)/l);
        normX2=sqrt(sum((abs(W2.*X)).^2,2));
        n12=sum(weights1.*normX2);
        ...
        for i=1:l
            ...
            n12=n12+weights1(i)*norm(W2(indice+1:indice+g_t(i)).*X(indice+1:indice+g_t(i)));
