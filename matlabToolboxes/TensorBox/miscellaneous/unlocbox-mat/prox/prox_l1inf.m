function [sol,infos] = prox_l1inf(x, gamma , param)
%PROX_L1inf Proximal operator with L1inf norm
%   Usage:  sol=prox_l1inf(x, gamma, param)
%           [sol,infos]=prox_l1inf(x, gamma, param)
%   Input parameters:
%         x     : Input signal.
...
%
%      sol = argmin_{z} 0.5*||x - z||_2^2 + gamma * ||A x||_1inf
%
%
...
infos.iter=iter;
infos.final_eval=norm_l1inf(sol,param.g_d,param.g_t);
infos.crit=crit;
infos.time=toc(t1);
