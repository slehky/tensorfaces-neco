...
x(I(1:K)) = rand(K, 1); % take a uniform distribution (adapted to L1inf norm)
x = x/norm(x);
...
f1.prox=@(x, T) prox_l1inf(x, T, param2);
f1.eval=@(x) norm_l1inf(x,g_d,g_t);      



% solving the problem
sol=douglas_rachford(zeros(N,1),f1,f2,param);
