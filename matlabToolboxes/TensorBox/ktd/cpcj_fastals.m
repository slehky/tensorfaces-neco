function [P,output] = cpcj_fastals(X,R,opts)
% Fast ALS for conjugate CP factorizes the N-way tensor X into factors of R components.
%   
% 
% INPUT:
...
%
% OUTPUT: 
%  P:  ktensor of estimated factors , P is conjigate with set of tensors
%  T_k (orthogonal with T_k.
%  output:  
%      .Fit
%      .NoIters 
%
...
if (nargin == 0) || ((nargin ==1) && isstruct(opts))
    P = param; return
end
...
%%
conj_projection = false;
if ~isempty(param.Yref) 
    ell_ref = sum(abs(param.Yref).^2);
    act_ref = ell_ref>0;
    if act_ref
         conj_projection = true;
         Yref = param.Yref(:,act_ref);
         K_ref = size(Yref,2);
    end
end
...
if any(bestord~=(1:N))
    ...
    if conj_projection 
        Yref = reshape(Yref,[I K_ref]);
        Yref = permute(Yref,[p_perm numel(p_perm)+1]);
        Yref = reshape(Yref,[],K_ref);
    end 
    I = I(p_perm);
end
...
for iter = 1:param.maxiters
    ...
    for n = updateorder(1:end) %[1 3 2] %
        ...
        % Projection U_n 
        if conj_projection
            W = zeros(numel(U{n}),K_ref);
            for kf = 1:K_ref
                temp = mttkrp(tensor(Yref(:,kf),I),U,n);
                W(:,kf) = double(temp(:));
            end
            [W,rr] = qr(W,0);
            ...
            % Projectioni 2
            Wg = reshape(W,[],R,K_ref);
            Wg = reshape(permute(Wg,[1 3 2]),[],R);
            %Wg = reshape(W',[],R);
            Wg = Wg/Gamma;
            Wg = ipermute(reshape(Wg,[],K_ref,R),[1 3 2]);
            Wg = reshape(Wg,[],K_ref);
            
            U{n}(:) = U{n}(:) - Wg*((W'*Wg)\(W'*U{n}(:)));
        end
