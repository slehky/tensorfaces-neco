...
% Generate data with 70% of missing pixels
% W is a tensor indicator of 0 and 1. Missing entries are set to nan.

missing_ratio = .7; nopix = prod(I(1:2));
...
switch configuration
    case '1s' % Configuration 1s:  Single group KTC
        Ix = [8 8 1];    % size of patterns X 
        
        opts.step = [];  % shift interval
        
    case '2s' % Configuration 2s: Single group KTC with sequential shifts of 
              % +/-4 pixels for each dimension
        Ix = [8 8 1];    % size of patterns X
        
        opts.step = [4 4];  % shift interval
        opts.shift_type = 'sequential';
        
    case '3s' % Configuration 3s: Single group KTC for the augmented data 
              % from all shifts +/-4 pixels for each dimension.
        Ix = [8 8 1];    % size of patterns X 
        
        opts.step = [4 4];  % shift interval
        opts.shift_type = 'augmented';
        
    case '1m' % Configuration 1m:  multi group KTC
        Ix = [4 4 1; 8 8 1];     
        
        opts.step = [];  % shift interval
        opts.multigroup_type = 'simultaneous';
        
    case '2m' % Configuration 2m:  multi group KTC as multiple single group KTCs.
              % The KTC decomposes the data with differnt pattern sizes. 
              % The final image is averaged or takedn median values of
              % approximate images.
              
        Ix = [4 4 1; 8 8 1];     
        opts.step = [];  % shift interval
        opts.multigroup_type = 'sequential';
        
    case '3m' % Configuration 3m:  multi group KTC with sequential shift in
              % an interval of [9 x 9].
    ...
    case '4m' % Configuration 4m:  multi group KTC with sequential shift in
              % an interval of [9 x 9]. Different from configuration 3m,
              % for this setting, KTC considers multiple groups as
              % decompositions with single groups of patterns.
    ...
    case '5m' % Configuration 5m:  multi group KTC for augmented data from 
              % all shifts 
    ...
    case '6m' % Configuration 5m:  multi group KTC for augmented data from 
              % all shifts, but patterns are considered as single group. 
...
Ia = bsxfun(@rdivide,I,Ix);
P = min([prod(Ia,2),prod(Ix,2)],[],2);
