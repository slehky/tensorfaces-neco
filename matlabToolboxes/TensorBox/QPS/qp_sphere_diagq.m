function [x,fval,lda] = qp_sphere_diagq(s,b,solving_method,verbosity)
% Quadratic programing with spherical constraints
%    min f(x) = 1/2*x'*diag(s)*x + b'*x
%    subject to  x'*x = 1
%
% where Q = diag(s) is a positive semi-definite matrix of size K x K
% b is a Kx1 vector.
%
...
%% Group coefficients c_k if some s_k are identical 
s0 = s; c0 = c;K0 = K;
...
%% If there are identical eigenvalues, need to return back to the full solution (non-reduction)
if K<K0
    x = Asimp*x;
end
%%
        ...
        if abs(lda_2-lda_1)<=1e-15
            ...
            switch solving_method
                case 'fzero'
                    ...
                    %options = optimset('fzero'); % this line takes time 
                ...
                case 'fminbnd'
                    ...
                    while abs(f2-f1)>1e-30 || abs(gval) > options.TolX
